...
def main() -> None:
    ...
    if len(config_dirs) != 4:
        raise RuntimeError(f"Expected four trainable configs for {seed_name}.")
    if not args.smoke and args.seed == 0:
        deterministic = sorted(
            path.parent
            for path in args.experiment_root.glob(
                "heston-xi*/public/deterministic/*/config.toml"
            )
        )
        if len(deterministic) != 4:
            raise RuntimeError("Expected four deterministic configs.")
        config_dirs = deterministic + config_dirs
