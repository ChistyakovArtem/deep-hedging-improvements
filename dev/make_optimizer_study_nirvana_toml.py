...
from dev.generate_optimizer_study import EXPERIMENT_ROOT, SEEDS
    ...
    return [
        {
            **PROFILE,
            "comment": (
                "ICAIF26-OptimizerStudy-5Families-5HP-2Markets-"
                f"Seed{seed}-TabularNormal-Jul24-v1"
                + ("-Smoke" if smoke else "")
            ),
            "main_commands": (
                "uv run --no-sync python -m bin.run_optimizer_study_seed "
...
def validate_graphs(graphs: list[dict], smoke: bool = False) -> None:
    ...
    if sum(int(graph["n_gpus"]) for graph in graphs) != expected:
        raise RuntimeError("Each optimizer queue must use exactly one GPU.")
    ...
    for seed, graph in enumerate(graphs):
        ...
        if graph["priority"] != "normal":
            raise RuntimeError("Full optimizer runs require tabular-normal.")
...
def main() -> None:
    ...
    if output is None:
        filename = (
            "icaif26-optimizer-study-smoke.toml"
            if args.smoke
            else "icaif26-optimizer-study-8x1gpu-tabular-normal.toml"
        )
