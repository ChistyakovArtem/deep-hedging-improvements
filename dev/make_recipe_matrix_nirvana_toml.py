...
from dev.generate_recipe_matrix import EXPERIMENT_ROOT, SEEDS
    ...
    return [
        {
            **PROFILE,
            "comment": (
                "ICAIF26-RecipeMatrix-8Variants-2Markets-"
                f"Seed{seed}-16Configs-TabularNormal-Jul24"
            ),
            "main_commands": (
                "uv run --no-sync python -m bin.run_recipe_matrix_seed "
...
def validate_graphs(graphs: list[dict]) -> None:
    ...
    if sum(int(graph["n_gpus"]) for graph in graphs) != 8:
        raise RuntimeError("The recipe matrix must request exactly eight GPUs.")
    ...
    for seed, graph in enumerate(graphs):
        ...
        if graph["n_gpus"] != 1:
            raise RuntimeError("Each seed queue must use one GPU.")
        ...
        if graph["data_ids"] or graph["data_commands"] != "mkdir -p data":
            raise RuntimeError("Leaderboards and reference grids must be generated in-job.")
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("local/nirvana/icaif26-recipe-matrix-8x1gpu-tabular-normal.toml"),
    )
    ...
    print(f"wrote {len(graphs)} balanced one-GPU seed queues to {args.output}")
